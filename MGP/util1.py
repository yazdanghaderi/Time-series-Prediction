...
def pad_rawdata(T, Y, ind_kf, ind_kt, X):
    """
    ...
    # grid_lens = np.array([len(X[m]) for m in X])
    grid_lens = np.array([np.shape(m)[0] for m in X])
    ...
    return T_pad, Y_pad, ind_kf_pad, ind_kt_pad, X_pad
...
def block_Lanczos(Sigma_func, B_, n_mc_smps):
    ...
    for ss in range(s):
        ...
        E_sqrt = tf.concat(
            [E_sqrt, tf.expand_dims(tf.diag(tf.squeeze(tf.sqrt(tf.maximum(tf.slice(E, [ss, 0], [1, -1]), 1e-6)))), 0)],
            0)
