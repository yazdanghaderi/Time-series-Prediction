...
if __name__ == "__main__":
    ...
    with g.as_default():
        ...
        preds,ZZ = get_preds(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values, num_rnn_grid_times)
        ...
        saver.restore(sess, "checkpoint4444_4layer_smp35_hiiden30_dropout0.2/MGP-209792")
        ...
        for i in range(training_iters):
            ...
            if i % 5 == 0:
                ...
                # saved_path = saver.save(sess, 'checkpoint4444_4layer_smp35_hiiden30_dropout0.2/MGP', global_step=global_step)
                print("\tnum_epoch:", i)
                test_t = time()
                ...
                ZZ = sess.run(ZZ, feed_dict1)
                ZZ=np.array(ZZ)
                SS=np.zeros([35,30,5])
                NN=np.zeros([1,30,5])
                PP=np.zeros(([200,30,5]))
                for i in range(0,200):
                    # for j in range(0,35):
                    SS=SS+ZZ[(i * 35):((i + 1) * 35), :, :]
                    for j in range(35):
                        NN=NN+SS[j,:,:]
                    NN=NN/35
                    PP[i,:,:]=NN
                ...
                Ps=np.array(Ps)
                Ps=Ps.reshape([30,5])
                Ps=sc.inverse_transform(Ps)
                print(Ps)

                dd=[]
                dd1=[]
                for i in range(19100 - 30, 19300 - 30):
                    # dd.append(dataset[i - 30:i, 0:5])
                    dd.append(traning_set_scaled[i - 30:i, 0:5])
                dd=np.array(dd)
                dd1=dd[0,:,:]
                dd1=np.array(dd1)
                dd1=dd1.reshape([30,5])
                dd1=sc.inverse_transform(dd1)
                print(dd1)


                        # print(np.shape(ZZ[(i*35):((i+1)*35),:,:]))
                        # SS=SS+ZZ[0:2,29,3]
                # print(np.shape(ZZ))
                ...
                # RMSE = mean_absolute_error(Real_energy_consumtion, predicted_energy_consumtion)
