...
def get_preds(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values,
    ...
    # Z.set_shape([None, n_mc_smps * 30* input_dim])  # somehow lost shape info, but need this
    Z.set_shape([None, None,input_dim])
    # Z=tf.reshape(Z,[None,150])# somehow lost shape info, but need this
    ...
    W1 = tf.Variable(tf.random_normal([150, 50], stddev=(1 / tf.sqrt(float(150)))), name='W1')
    b1 = tf.Variable(tf.random_normal([50]), name='b1')

    W2 = tf.Variable(tf.random_normal([50, 50], stddev=(1 / tf.sqrt(float(50)))), name='W2')
    b2 = tf.Variable(tf.random_normal([50]), name='b2')

    # W3 = tf.Variable(tf.random_normal([80, 80], stddev=(1 / tf.sqrt(float(80)))), name='W3')
    # b3 = tf.Variable(tf.random_normal([80]), name='b3')

    W33 = tf.Variable(tf.random_normal([50, 100], stddev=(1 / tf.sqrt(float(50)))), name='W33')
    b33 = tf.Variable(tf.random_normal([100]), name='b33')

    # hidden layer1-to-output
    W4 = tf.Variable(tf.random_normal([100, 1], stddev=(1 / tf.sqrt(float(100)))), name='W4')
    b4 = tf.Variable(tf.random_normal([1]), name='b4')
    # ######################## Activations, outputs ######################
    # output hidden layer 1
    hidden_out1 = tf.nn.relu(tf.add(tf.matmul(Z, W1), b1))
    hidden_out2 = tf.nn.sigmoid(tf.add(tf.matmul(hidden_out1, W2), b2))
    # hidden_out3 = tf.nn.sigmoid(tf.add(tf.matmul(hidden_out2, W3), b3))
    hidden_out33 = tf.nn.sigmoid(tf.add(tf.matmul(hidden_out2, W33), b33))

    # flatt=tf.reshape(flatt,[n_mc_smps*batch_size,-1])
    # flatt_shape1=tf.shape(flatt)[1]

    y_ = tf.nn.sigmoid(tf.add(tf.matmul(hidden_out33, W4), b4))
    out1 = tf.reshape(y_, [-1, 1])
...
if __name__ == "__main__":
    ...
    training_iters = 30  # num epochs=25
    ...
    n_mc_smps = 25  #####fek konam manzor sampelhay monte carlo ast!!!!!!!!!
    ...
    with g.as_default():
        ...
        out1 = get_preds(Y, T, X, ind_kf, ind_kt, num_obs_times, num_obs_values, num_rnn_grid_times)
        ...
        prediction = get_probs_and_accuracy(out1)

        loss = tf.reduce_sum(tf.square(out1 - target_target))
        ...
        # saver.restore(sess, "/home/diako/Documents/Desktop_day97/MGP-RNN-master/MGP-RNN-master/MGP-multi-layer-MLP/mgp-571440")
        for i in range(training_iters):
            ...
            for s, e in zip(starts, ends):
                ...
                # print(sess.run(out1_shape,feed_dict))
            ...
            # saved_path = saver.save(sess, 'MGP-multi-layer-MLP/mgp', global_step=global_step)
